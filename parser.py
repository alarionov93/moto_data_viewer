...
parser.add_argument('--truncate', action='store_true', help='Truncate log file to store only new values into db and exit')
...
res = cursor.fetchone()
print(res)
if args.get("new") == True and args.get("truncate") == True:
    print("Only one of these options should be passed!")
else:
    ...
    if  args.get("force") == True:
        ...
        if args.get("truncate") == True and args.get("new") == False:
            # TODO: truncate log file here
            with open(DEFAULT_LOG_FILENAME, "w") as f:
                f.truncate()
        else:
            new_measure_id = None
            if res[0] is not None:
                last_measure_id = int(res[0])
                if args.get("new") == True and args.get("truncate") == False:
